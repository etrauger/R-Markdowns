...
nnn <- ncol(X.train)
mod1 %>%
  layer_dense(units=nnn+20, activation='relu', input_shape=nnn) %>%
...
lr_schedule <- function(epoch, lr, base_lr = 0.000005, max_lr = 0.001, step_size = round(eps/10), gamma=0.995) {
  cycle <- floor(1+epoch/(2*step_size))
  x <- abs(epoch/step_size - 2*cycle + 1)
  lrn <- base_lr + (max_lr-base_lr)*max(c(0, (1-x)))*(gamma^epoch)
  return(lrn)
}
ln <- NULL
for (i in 1:1000) {
