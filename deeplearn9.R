...
## Initial Variables ##
# date variables
date.end <- Sys.Date()-14
date.start <- date.end  - 360*9
...
for (q in 1:ncol(data)) {
  wt1 <- analyze.wavelet(my.data=data, my.series=q, loess.span=0.001)
  newplot <- as.xts(wt1$series)[,2]
  trans.data <- merge.xts(trans.data, newplot)
}
plot(as.xts(data))
plot(trans.data)
plot(as.xts(data) - trans.data)
data <- trans.data
...
mod1 %>%
  ...
  layer_cudnn_lstm(units=round(factor_cols*factor_depth/2),
                   return_sequences=TRUE) %>%
  layer_batch_normalization() %>%
  layer_dropout(rate=0.3) %>%
...
mod1 %>% compile(
  loss='mse',
  optimizer=optimizer_nadam(lr=0.0004),
  metrics=c('mae')
)
...
## Input values ##
eps <- 3000
maximum_height <- 0.005
minimum_height <- 0.00005
number_of_spikes <- 2
...
mod1.hst <- mod1 %>% fit(
  x=X.train, y=Y.train,
  epochs=eps, batch_size=round(factor_rows/10),
