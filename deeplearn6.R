...
format_output <- function(inpdata, forecast_days) {
  outdata <- inpdata
  labels_for_output <- c("t_0")
  for (k in 1:forecast_days) {
    kday_lag <- lag(inpdata, k=k)
    outdata <- merge.xts(outdata, kday_lag)
    newlab <- paste("t_", k, sep="")
...
format_input <- function(inpdata, forecast_days, lag_days) {
  ...
  for (r in 1:ncol(inpdata)) {
    ...
    for (m in lagstart:lag_days)  {
      ...
      newlabel <- paste(colnames(inpdata[,r]), "_", m, sep="")
      labels_for_output <- c(labels_for_output, newlabel)
    }
    colnames(data_append) <- labels_for_output
    outputdata <- merge.xts(outputdata, data_append)
  }
  return(outputdata)
}
...
training.percent <- 0.85      # 85%
validation.percent <- 0.10     # 10%
nlags <- 60
forecast_days <- 20
...
target_data <- format_output(data$X10Y.Yield, forecast_days)
total_data <- merge.xts(target_data, indicator_data)
total_data <- removeNA(total_data)

trn <- floor(nrow(total_data) * training.percent)
mod1.data <- data.matrix(total_data)
...
## training data ##
trncols <- (forecast_days+2):ncol(mod1.data)
tstcols <- 1:(forecast_days+1)
X.train <- mod1.data[1:trn, trncols]
Y.train <- mod1.data[1:trn, tstcols]

vl <- floor((training.percent+validation.percent)*nrow(total_data))
X.val <- mod1.data[(trn+1):vl, trncols]
Y.val <- mod1.data[(trn+1):vl, tstcols]

ts <- nrow(total_data)
X.test <- mod1.data[(vl+1):ts, trncols]
Y.test <- mod1.data[(vl+1):ts, tstcols]
...
nnn <- ncol(X.train)
mod1 %>%
  layer_dense(units=nnn+20, input_shape=nnn, kernel_regularizer=regularizer_l2(0.01)) %>%
  layer_activation_leaky_relu() %>%
  layer_dense(units=nnn*5, kernel_regularizer=regularizer_l2(0.01)) %>%
  layer_activation_leaky_relu() %>%
  layer_dense(units=nnn, kernel_regularizer=regularizer_l2(0.001)) %>%
  layer_activation_leaky_relu() %>%
  ...
  layer_dense(units=nnn, kernel_regularizer=regularizer_l2(0.01)) %>%
  layer_activation_leaky_relu() %>%
  layer_dense(units=nnn*2, kernel_regularizer=regularizer_l2(0.01)) %>%
  layer_activation_leaky_relu() %>%
  layer_dense(units=nnn, kernel_regularizer=regularizer_l2(0.01)) %>%
  layer_activation_leaky_relu() %>%
  layer_dense(units=nnn*5, kernel_regularizer=regularizer_l2(0.01)) %>%
...
rownames(mod1.preds) <- rownames(Y.test)
test.resize.mean <- train.mean[tstcols]
test.resize.std  <- train.std[tstcols]
mod1.preds <- mod1.preds*test.resize.std + test.resize.mean
Y.test <- Y.test*test.resize.std + test.resize.mean

## Format predictions  vs. Actuals ##
daily_prediction_error <- c()
for (i in 1:nrow(mod1.preds)) {
  pred.data <- mod1.preds[i,]
  real.data <- Y.test[i,]
  sspe <- sum((real.data - pred.data)^2)
  daily_prediction_error[i] <- sspe
  names(daily_prediction_error[i]) <- rownames(mod1.preds)[i]
}

daily_prediction_error <- as.xts(daily_prediction_error, order.by = as.Date(rownames(Y.test)))


preds.plot <- as.matrix(mod1.preds[nrow(mod1.preds),])
alldates <- index(total_data)
newindex <- which(alldates == rownames(Y.test)[nrow(Y.test)])
newindex <- alldates[(newindex-forecast_days):(newindex)]
preds.plot <- as.xts(preds.plot[,1], order.by=newindex)
newindex <- which(alldates == rownames(Y.test)[nrow(Y.test)])
altindex <- alldates[(newindex-forecast_days*10):(newindex)]
Y.plot <- data$X10Y.Yield[altindex]
plot.obj <- merge.xts(Y.plot, preds.plot)


## Setup system to predict in the future ##
result_data <- NULL
usedata <- data
totdata <- NULL
for (i in 1:ncol(usedata)) {
  dat.labels <- c(colnames(usedata)[i])
  outdata <- usedata[,i]
  for (j in 1:(nlags-(forecast_days+1))) {
    nd <- lag(usedata[,i], k=j)
    outdata <- merge.xts(outdata, nd)
    lab.append <- paste(colnames(usedata)[i], "_", j, sep="")
    dat.labels <- c(dat.labels, lab.append)
  }
  colnames(outdata) <- dat.labels
  totdata <- merge.xts(totdata, outdata)
}

input_dataset <- totdata[nrow(totdata),]
input_dataset <- scale(input_dataset, center=train.mean[trncols], scale=train.std[trncols])

new_preds <- mod1 %>% predict(input_dataset)
new_preds <- new_preds* train.std[tstcols] + train.mean[tstcols]
new_preds <- t(as.matrix(new_preds))
newindex <- index(input_dataset)
...
newindexset <- newindexset[1:length(new_preds)]

new_preds <- as.xts(new_preds, order.by=newindexset)
plot.obj <- merge.xts(Y.plot, preds.plot, new_preds)
...
legend("top", inset=0.03,
       legend=c("Training", "Testing", "Future Prediction"),
       col=c("black", "green", "purple"),
       lty=1, lwd=2)
