...
mod1 %>%
  layer_dense(units=5000, activation='relu', input_shape=ncol(X.train)) %>%
...
# reduce_lr <- callback_reduce_lr_on_plateau(factor = 0.9, 
#                                            patience = 5, 
#                                            verbose = 1, 
#                                            mode = "auto", 
#                                            min_lr = 0.000005)
...
mod1.hst <- mod1 %>% fit(
  x=X.train, y=Y.train,
  epochs=75, batch_size=nrow(X.train),
...
model_data <- as.data.frame(mod1.data)
pred.data <- model_data$X10Y.Yield

model_components <- list()
model_components <- AddLocalLinearTrend(model_components, y=pred.data)
model_components <- AddSeasonal(model_components, y=pred.data, nseasons=52, season.duration = 5)

model_data <- as.matrix(model_data)
colnames(model_data) <- NULL
model_data <- as.data.frame(model_data)
colnames(model_data)[ncol(model_data)] <- "X10Y.Yield"

model_fit <- bsts(pred.data,
                  state.specification= model_components, 
                  niter=1000)

model_prediction <- predict(model_fit, horizon=100, quantiles=c(0.05, 0.95))
